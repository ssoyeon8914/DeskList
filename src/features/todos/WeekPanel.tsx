...
import type { AppState, WeekStartsOn } from "../../domain/types";
import {
  applyWeekDateFilter,
  setFilterGroup,
  setWeek,
  ...
  return (
    <section className="week-panel" aria-label="주간 달력">
      ...
      <div className="week-grid" aria-label="주간 그리드">
        ...
        {cells.map((c) => {
          const items = list.filter((t) => t.date === c.date);
          let selCls = " wc--on";
          if (!fullWeek) {
          ...
          return (
            ...
            >
              ...
              {items.slice(0, 3).map((t) => (
                <span key={t.id}>
                  {iconFor(state.types, t.type)} {t.title}
                  <br />
                </span>
              ))}
              {items.length > 3 && (
                <span className="badge-num">+{items.length - 3}</span>
              )}
            </div>
          );
        })}
